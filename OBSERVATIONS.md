...
The spec prevents two tickets with the same dispatch time for the same truck. What it doesn't address is that two tickets for the same truck, one second apart, are equally impossible. A service-layer guard with a configurable minimum gap (e.g. 15 minutes) between tickets for the same truck would enforce this. I did not implement anything on this because there is no operational data, and so I chose to keep the API simple.
...
The bulk create endpoint is not idempotent. This matters because networks are unreliable: if a client sends a request, the server processes it successfully, but the response never arrives (network drop, timeout), the client has no way to know whether the tickets were created. and retries would cause issues.
...
No per-client throttle exists on the bulk create endpoint. A single client could hammer the endpoint repeatedly.

---

**Authentication**

All endpoints are open. In production, a JWT guard on every route. Would also help with inserting user credentials for things like RBAC and certain endpoints in a large codebase.

---

**Date tolerance is an interpretation**

The spec states "tickets cannot be dispatched at a future date." The implementation rejects timestamps more than 60 seconds ahead of the server clock, not strictly any future timestamp, to account for client clock drift. TThis was a judgement call.

In the same vein, the spec says nothing about how far in the past a dispatcher can log. The current implementation allows any past timestamp.
